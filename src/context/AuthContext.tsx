...
interface AuthContextType {
  ...
  login: (email: string, username: string, accessCode: string) => Promise<{ success: boolean; error?: string }>;
  logout: () => void;
}
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, username: string, accessCode: string): Promise<{ success: boolean; error?: string }> => {
    try {
      // Debug logs
      ...
      if (!VALID_ACCESS_CODES.includes(formattedAccessCode)) {
        console.log('Invalid access code:', formattedAccessCode);
        return {
          success: false,
          error: 'Kode akses tidak valid. Silakan periksa kembali kode akses Anda.'
        };
      }
